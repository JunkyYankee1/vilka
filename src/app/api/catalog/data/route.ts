...
type CatalogRow = {
  ...
  // фактическая категория, к которой привязано блюдо
  ref_category_id: number;
  category_level: number;

  // нормализованный путь (всегда 0–3 уровней, по parent_id)
  ...
  try {
    const { rows } = await query<CatalogRow>(
      ...
      SELECT
        ...
        -- ===== Нормализуем путь категории по уровням =====
        -- c  - фактическая категория (1 / 2 / 3)
        -- p1 - родитель c
        -- p2 - родитель p1
      ...
      -- её родитель и родитель родителя
    ...
    type Category = { id: string; name: string; isPromo?: boolean };
    ...
    for (const row of rows) {
      // --- Определяем фактический level1 / level2 / "лист" для этого блюда ---
      ...
      // categoryId = всегда корневой уровень
      const categoryId = level1Code;

      // subcategoryId:
      // - если есть настоящий 2-й уровень → он
      // - если нет → делаем "виртуальную подкатегорию" = сама корневая
      ...
      // --- Заполняем справочники, если ещё нет ---
