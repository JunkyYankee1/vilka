...
> Важно: в текущей схеме БД `users.phone` **NOT NULL + UNIQUE**, поэтому для Telegram‑пользователей используется плейсхолдер `phone = tg:<telegram_id>`. В UI это скрыто (показывается Telegram username/имя).

## Настройка

1) Создай бота через `@BotFather`, включи Login Widget и задай домен (в BotFather: `/setdomain`).

2) Добавь в `.env.local` (для Next.js/app контейнера):

```bash
TELEGRAM_AUTH_BOT_TOKEN=123456:ABCDEF...
```

3) Перезапусти `app` контейнер / dev-сервер.
